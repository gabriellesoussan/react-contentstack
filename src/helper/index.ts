...
import { BannerRes, FooterRes, HeaderRes } from "../typescript/response";
...
export const getBannerRes = async (): Promise<BannerRes> => {
  const response = (await getEntry({
    contentTypeUid: "banner",
    referenceFieldPath: ["button.url"],
    jsonRtePath: undefined,
  })) as BannerRes[][];
  liveEdit && addEditableTags(response[0][0], "banner", true);
  return response[0][0];
};
